...
- **Firmware (C6, currently the board in the headset):** `Nami_Headset_LEDs_C6/Nami_Headset_LEDs_C6.ino`, compiles/flashes via `arduino-cli` over COM8 with FQBN `esp32:esp32:XIAO_ESP32C6:PartitionScheme=no_ota` (this board has no `min_spiffs` option; SPIFFS isn't used by this project anyway). Storage usage was 42% of the 2MB app partition as of 2026-08-14. `LED_PIN` is GPIO18 (D10), `STRIP_LEDS` is 69. QA defaults are blue/brighter — see 2026-08-17 entry above.
