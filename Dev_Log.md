...
### 2026-08-18 — C6/webpage cleanup, classic ESP32 archived
**Files:** `Nami_Headset_LEDs_C6/Nami_Headset_LEDs_C6.ino`, `index.html`, `CLAUDE.md`, `.claude/skills/rebuild-and-flash/SKILL.md`
- **Cleanup pass** (requested: "review C6 firmware and index, clean up and reduce inefficiencies"), no behavior change:
  - `pctTo255(pct, floor255)`: the `floor255` parameter was always called with `0` at all 10 call sites, and since the return type is `uint8_t` (unsigned), `max(v, 0)` was a no-op even when it did run — dead flexibility that was never actually exercised. Simplified to `pctTo255(pct)`, single parameter.
  - `index.html`: deleted a block of commented-out old `onDisconnect()` code kept "for reference/rollback" — the current version has been stable for many sessions, no reason to keep dead code around.
  - `index.html`: extracted the RSSI-subscription block (get characteristic → `startNotifications()` → listener, with its try/catch fallback for older firmware) into one `subscribeRssi(service)` helper — was duplicated verbatim in `toggleConnect()` and `attemptReconnect()`.
  - `index.html`: extracted the pattern-switch UI logic (tuning panel title from a `QA_NAMES` array, field visibility by `data-qa`, show the divider) into one `showPatternUI(idx)` helper — was duplicated verbatim in `selectQA()` and `applyState()`.
  - Verified: compiled to the identical size as before (887,398 bytes / 42%, confirming this was pure duplication/dead-code removal, nothing behavioral), webpage loaded with zero console errors in the Browser pane preview, flashed to the C6 over COM8 with a clean boot log.
- **Classic ESP32 board retired.** User is archiving the classic ESP32 hardware — no more parallel maintenance of two firmwares going forward. Chose to leave `Nami_Headset_LEDs/Nami_Headset_LEDs.ino` untouched in the repo as a historical/reference build (not deleted, not moved) rather than actively archiving the file itself. Updated `CLAUDE.md` ("Key files" and "Firmware toolchain" sections now clearly separate the active C6 board from the archived classic one; also fixed a stale "228 LEDs / 120-LED strip" line in "What this is" that hadn't been true since the strip was physically shortened — corrected to 176 LEDs / 69-LED strip) and `.claude/skills/rebuild-and-flash/SKILL.md` (now defaults to the C6 board without asking which one — it had reverted back to classic-only when the OTA work was stashed, since that update was part of the same stashed changeset).

---

## Current state (as of latest files on hand)
- **Patterns:** Solid, Breathe, Waves, Rainbow — all 4 implemented. Waves/Rainbow strip rendering is safe for odd LED counts (rounds the midpoint up, not down) and Waves' trail auto-caps to fit the travel distance so its edge-to-middle reset never shows a visible pop.
- **Connection:** auto-reconnect + ping-based fast disconnect detection + cancelable reconnect UI, all in place in `index.html`. RSSI subscription and pattern-switch UI logic are now each in a single shared helper rather than duplicated (2026-08-18 cleanup).
...
- **Firmware — ESP32-C6 is the sole active board** (classic ESP32 archived 2026-08-18): `Nami_Headset_LEDs_C6/Nami_Headset_LEDs_C6.ino`, compiles/flashes via `arduino-cli` over COM8 with FQBN `esp32:esp32:XIAO_ESP32C6:PartitionScheme=no_ota`. Storage usage 42% of the 2MB app partition as of 2026-08-18. `LED_PIN` is GPIO18 (D10), `STRIP_LEDS` is 69. QA defaults — see the 2026-08-17 entries above.
- **Firmware — classic ESP32, archived/reference only:** `Nami_Headset_LEDs/Nami_Headset_LEDs.ino` is left in the repo untouched but is no longer kept in sync with the C6 build. Last known-good state: FQBN `esp32:esp32:esp32:PartitionScheme=min_spiffs`, COM7, 63% of the 1.9MB app slot as of 2026-08-08, `STRIP_LEDS` 68, teal/dimmer QA defaults (never updated to match the C6's later look changes).
