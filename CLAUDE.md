...
## What this is
An ESP32-based LED headset (cosplay use). 174 WS2812B LEDs (RingA 54 / Strip 66 / RingB 54 — the strip was physically shortened from its original 120) controlled over BLE from a self-contained webpage (Web Bluetooth) hosted on GitHub Pages so it's reachable from a phone away from any local server.
